...
sudo echo anskattah1122:anskattah1122| chpasswd
sudo sed -i "s/.*PasswordAuthentication no/PasswordAuthentication yes/g" /etc/ssh/sshd_config
sudo service sshd restart
sudo echo "%wheel  ALL=(ALL)       NOPASSWD: ALL" >> /etc/sudoers
sudo service sshd restart
sudo usermod -aG wheel anskaycekatta
...
sudo git clone -b installations https://github.com/Kelechiukatta/devops-fully-automated-infra.git /tmp/devops-fully-automated
